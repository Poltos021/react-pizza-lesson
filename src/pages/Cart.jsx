...
import { clearCart, removePizza, plusItem, minusItem } from '../redux/action/cart';
import cartEmpty from '../assets/img/empty-cart.png';
import { Link } from 'react-router-dom';

function Cart({name, type, size}) {
...
<span onClick={onClearCart}>Очистить корзину</span>
        ...
        <div className="content__items">
          {
            groupPizzas.map(obj => <CartItem 
              key={obj.id}
              id={obj.id}
              name={obj.name} 
              type={obj.type} 
              size={obj.size} 
              totalPriceThisPizza={items[obj.id].totalPrice} 
              totalCount={items[obj.id].items.length} 
              onRemove={removeOnPizza}
              onMinus={onMinusItem}
              onPlus={onPlusItem} />)
          }
        </div>
